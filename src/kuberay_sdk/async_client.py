...
class AsyncClusterHandle:
    ...
    async def wait_until_ready(self, timeout: float = 300) -> None:
        ...
        svc = ClusterService(self._client._custom_api, self._client._config)
        await _run_sync(svc.wait_until_ready, self._name, self._namespace, timeout)
...
class AsyncJobHandle:
    ...
    async def wait(self, timeout: float = 3600) -> Any:
        ...
        if self._mode == "DASHBOARD" and self._dashboard_url:
            ...
            dc = DashboardClient(self._dashboard_url)
            return await _run_sync(svc.wait_dashboard_job, dc, self._name, timeout)
        return await _run_sync(svc.wait, self._name, self._namespace, timeout)
